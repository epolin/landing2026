...
¡Gracias!`)}`;window.open(t,"_blank")}prefillFormFromURL(){const e=new URLSearchParams(window.location.search);["nombre","empresa","email","telefono"].forEach(t=>{const a=e.get(t),i=document.getElementById(t);a&&i&&(i.value=decodeURIComponent(a))})}trackUserInteraction(e,o={}){const t={timestamp:new Date().toISOString(),action:e,details:o,page:"Copa Mundial 2026 Landing",userAgent:navigator.userAgent,url:window.location.href};console.log("Interacción registrada:",t)}updateCSSPaths(){console.log("Actualizando rutas CSS, basePath:",this.basePath);const e=this.getAssetPath("/bg/HeroBG.png"),o=this.getAssetPath("/bg/BeneficiosBG.png"),t=this.getAssetPath("/bg/ReservaBG.png");console.log("Rutas de imágenes:",{heroBg:e,beneficiosBg:o,reservaBg:t});const a=document.createElement("style");a.textContent=`
      .hero {
        background: url('${e}') center center / cover no-repeat !important;
      }
      .beneficios {
    ...
    `,document.head.appendChild(a),console.log("CSS dinámico aplicado")}}const y=`
